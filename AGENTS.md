...
Additional providers, Pull Request integration, deployment, and remote or multi-user Web UI are outside the MVP. The loopback-only Local Browser Control Plane described by FR-190 through FR-214 is a v1 extension and must remain an inbound adapter to the same Application Service used by the CLI.
